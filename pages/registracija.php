...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    ...
    if ($stmt->num_rows > 0) {
      ...
      $ins->bind_param(
        "ssssisssssii",
        $ime, $prezime, $email, $korisnicko_ime,
        $drzava_id, $grad, $ulica, $datum_rodenja,
        $hash, $rola, $user_cms, $aktivan
      );
...
<section id="registracija">
  ...
  <?php if ($success): ?>
    <p style="background: rgba(0,128,0,0.25); padding: 10px; border-radius: 8px;">
